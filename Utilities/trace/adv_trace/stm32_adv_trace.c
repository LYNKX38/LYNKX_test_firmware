...
{
  if (ADV_TRACE_Ctx.CurrentVerboseLevel == 0)
  {
      return;
  }
  ...
  /* check verbose level */
  if( VerboseLevel > ADV_TRACE_Ctx.CurrentVerboseLevel )
  {
  ...
  return HAL_UART_Transmit(&huart1, (uint8_t *)buf, buff_size,100);
...
static void TRACE_UnLock(void)
{
  UTIL_ADV_TRACE_ENTER_CRITICAL_SECTION();
  ADV_TRACE_Ctx.TraceLock--;
  //  ADV_TRACE_Ctx.TraceLock=0; //CMO force trace unlock
  UTIL_ADV_TRACE_EXIT_CRITICAL_SECTION();
}
