...
//#define APP_LOG(TS,VL,MSG)   HAL_UART_Transmit(&huart1, (uint8_t *)MSG, strlen((uint8_t *) MSG),100);

//  #define APP_LOG(TS,VL,MSG)   do{ if(VL>=DEBUG_LEVEL) {HAL_UART_Transmit(&huart1, (uint8_t *)MSG, strlen((uint8_t *) MSG),100);} }while(0);
#define APP_LOG(TS,VL, ...)   do{ {UTIL_ADV_TRACE_COND_FSend(VL, T_REG_OFF, TS, __VA_ARGS__);} }while(0)
