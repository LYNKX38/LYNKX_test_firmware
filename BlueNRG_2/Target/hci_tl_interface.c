...
int32_t HCI_TL_SPI_Reset(void)
{
  ...
  HAL_Delay(5);
  // 🔁 Attendre que l'IRQ redescende → le module a terminé son auto-init
  ...
  // 🔄 Optionnel : flush une lecture SPI pour vider un éventuel paquet corrompu
  uint8_t dummy[255];
  HCI_TL_SPI_Receive(dummy, sizeof(dummy));
...
int32_t HCI_TL_SPI_Receive(uint8_t* buffer, uint16_t size)
{
  ...
  /* Read the header */
#if HARDWARE_VERSION >= 14
  HAL_SPI_TransmitReceive(&hspi1, header_master, header_slave, HEADER_SIZE, 100);
#else
  ...
  if (byte_count == 0) {
    // Pas de données, libérer CS et sortir proprement
    HAL_GPIO_WritePin(HCI_TL_SPI_CS_PORT, HCI_TL_SPI_CS_PIN, GPIO_PIN_SET);
  ...
  if(byte_count > 0)
  {
    ...
    {
#if HARDWARE_VERSION >= 14
  HAL_SPI_TransmitReceive(&hspi1, &char_00, (uint8_t*)&read_char, 1, 100);
#else
...
#if HARDWARE_VERSION >= 14
      HAL_SPI_TransmitReceive(&hspi1, buffer, read_char_buf, size, 100);
#else
...
#if HARDWARE_VERSION >= 14
  HAL_EXTI_GetHandle(&hexti1, EXTI_LINE_4);
