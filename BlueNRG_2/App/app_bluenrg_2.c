...
uint8_t  counter            = 0;
uint8_t  local_name[] = {AD_TYPE_COMPLETE_LOCAL_NAME,'C','L','E','M',' ','$'};
...
static uint8_t SampleAppRst(void);
...
void MX_BlueNRG_2_Init(void)
{
	...
	Ensure_SPI_Config_For_BLE();
  /* USER CODE BEGIN SV */
    BSP_BC_CmdSend(BLE_ON);
...
//  {
//    PRINT_DBG("0x%02x ", data_buffer[i]);
//  }
//  PRINT_DBG("\r\n");
//  sendData(data_buffer, Nb_bytes); // CMO ping-pong for debug
//  uint32_t last_time = SysTimeGetMcuTime().SubSeconds;
//  PRINT_DBG("before process : %d ms\r\n", last_time);
  uint8_t aDataBuffer[BLE_BUFFER_SIZE] ;
  uint8_t Nb_bytes_answer = 0;
  Nb_bytes_answer = ProcessRemoteCommand(data_buffer, Nb_bytes, aDataBuffer);
//  last_time = SysTimeGetMcuTime().SubSeconds;
//  PRINT_DBG("after process : %d ms\r\n", last_time);
  sendData(aDataBuffer, Nb_bytes_answer); // CMO ping-pong for debug
//  last_time = SysTimeGetMcuTime().SubSeconds;
//  PRINT_DBG("after send : %d ms\r\n", last_time);
...
//  bdaddr[2] = (uint8_t) (random_number[6]);
  ...
  bdaddr[0] = (uint8_t) (*(uint8_t *)(SecureElementGetDevEui()+7));
  bdaddr[1] = (uint8_t) (*(uint8_t *)(SecureElementGetDevEui()+6));
  bdaddr[2] = (uint8_t) (*(uint8_t *)(SecureElementGetDevEui()+5));
  bdaddr[3] = (uint8_t) (*(uint8_t *)(SecureElementGetDevEui()+4));
  bdaddr[4] = (uint8_t) (*(uint8_t *)(SecureElementGetDevEui()+3));
  bdaddr[5] = (uint8_t) (*(uint8_t *)(SecureElementGetDevEui()+2));

//   bdaddr[1] = (SecureElementGetDevEui());
//   bdaddr[2] = (SecureElementGetDevEui());
//   bdaddr[3] = (SecureElementGetDevEui());
//   bdaddr[4] = (SecureElementGetDevEui());
//   bdaddr[5] = (SecureElementGetDevEui());

//  memcpy1( &bdaddr, SecureElementGetDevEui( )+2, 6 );
...
/*******************************************************************************
* Function Name  : SampleAppRst.
* Description    : Reset BLE .
* Input          : None.
* Return         : Status.
*******************************************************************************/
static uint8_t SampleAppRst(void)
...
//      0x00, 0xFF, //Company identifier code (Default is 0x0030 - STMicroelectronics: To be customized for specific identifier)
//      0x01,       // ID
//      0x15,       //Length of the remaining payload
////      0xE2, 0x0A, 0x39, 0xF4, 0x73, 0xF5, 0x4B, 0xC4, //Location UUID
////      0xA1, 0x2F, 0x17, 0xD1, 0xAD, 0x07, 0xA9, 0x61,
//
//      0xf4,0x1b,0x74,0x83,0xee,0x50,0x6c,0x88,
//      0x38,0x43,0x94,0x33,0x40,0xeb,0xac,0x61,
//
...
//   };
  ...
  uint8_t adv_data[] = {
      ...
      0x00, 0xFE, // manufacturer ID
      OLNPayload[0],OLNPayload[1],OLNPayload[2],OLNPayload[3],OLNPayload[4],\
      OLNPayload[5],OLNPayload[6],OLNPayload[7],OLNPayload[8],OLNPayload[9],\
      OLNPayload[10],OLNPayload[11],OLNPayload[12],OLNPayload[13],OLNPayload[14],
...
static void User_Process(void)
{
  ...
  // CMO : RF testing purpose ONLY
    aci_hal_set_tx_power_level(1, 7);
    aci_hal_tone_start(19,0);
//    while(1)
//      {}
