...
int32_t hci_notify_asynch_evt(void* pdata)
{
  ...
  int32_t ret = 0;
  sprintf((char *)MSG, "➡️ hci_notify_asynch_evt()\r\n");
  HAL_UART_Transmit(&huart1, (uint8_t*)MSG, strlen((char*)MSG), 100);
  ...
  if (list_is_empty (&hciReadPktPool) == FALSE)
  {
    ...
    if (hciContext.io.Receive)
    {
      ...
      if (data_len > 0)
      {
          sprintf((char *)MSG, "📦 Received %d bytes: ", data_len);
          HAL_UART_Transmit(&huart1, (uint8_t*)MSG, strlen((char*)MSG), 100);
          for (int i = 0; i < data_len; i++) {
            sprintf((char *)MSG, "%02X ", hciReadPacket->dataBuff[i]);
            HAL_UART_Transmit(&huart1, (uint8_t*)MSG, strlen((char*)MSG), 100);
          }
          sprintf((char *)MSG, "\r\n");
          HAL_UART_Transmit(&huart1, (uint8_t*)MSG, strlen((char*)MSG), 100);
        ...
        {
          sprintf((char *)MSG, "✅ Packet verified. Enqueued to RX.\r\n");
          HAL_UART_Transmit(&huart1, (uint8_t*)MSG, strlen((char*)MSG), 100);
        ...
        {
          sprintf((char *)MSG, "❌ Packet rejected. Returned to pool.\r\n");
          HAL_UART_Transmit(&huart1, (uint8_t*)MSG, strlen((char*)MSG), 100);
      ...
      {
          sprintf((char *)MSG, "⚠️ No data received. Packet returned to pool.\r\n");
          HAL_UART_Transmit(&huart1, (uint8_t*)MSG, strlen((char*)MSG), 100);
        /* Insert the packet back into the pool*/
        list_insert_head(&hciReadPktPool, (tListNode *)hciReadPacket);
      }
