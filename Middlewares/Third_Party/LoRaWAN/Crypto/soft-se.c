...
static void PrintKey( KeyIdentifier_t key )
{
    ...
    if (retval == SECURE_ELEMENT_SUCCESS)
    {
        if (key == APP_KEY)
        {
            MW_LOG(TS_OFF, VLEVEL_H, "###### AppKey:      ");
        }
        else if (key == NWK_KEY)
        {
            MW_LOG(TS_OFF, VLEVEL_H, "###### NwkKey:      ");
        }
        else if (key == APP_S_KEY)
        {
            MW_LOG(TS_OFF, VLEVEL_H, "###### AppSKey:     ");
        }
        else if (key == NWK_S_KEY)
        {
            MW_LOG(TS_OFF, VLEVEL_H, "###### NwkSKey:     ");
        }
        else if (key == MC_ROOT_KEY)
        {
            MW_LOG(TS_OFF, VLEVEL_H, "###### MCRootKey:   ");
        }
        else if (key == MC_KE_KEY)
        {
            MW_LOG(TS_OFF, VLEVEL_H, "###### MCKEKey:     ");
        }
        else if (key == MC_KEY_0)
        {
            MW_LOG(TS_OFF, VLEVEL_H, "###### MCKey_0:     ");
        }
        else if (key == MC_APP_S_KEY_0)
        {
            MW_LOG(TS_OFF, VLEVEL_H, "###### MCAppSKey_0: ");
        }
        else if (key == MC_NWK_S_KEY_0)
        {
            MW_LOG(TS_OFF, VLEVEL_H, "###### MCNwkSKey_0: ");
        }
#if (!defined (LORAWAN_KMS) || (LORAWAN_KMS == 0))
        MW_LOG(TS_OFF, VLEVEL_H, "%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\r\n",
...
{
#if (defined (KEY_EXTRACTABLE) && (KEY_EXTRACTABLE == 1))
    MW_LOG(TS_OFF, VLEVEL_H, "###### OTAA ######\r\n");
    PrintKey(APP_KEY);
    PrintKey(NWK_KEY);
    MW_LOG(TS_OFF, VLEVEL_H, "###### ABP  ######\r\n");
    PrintKey(APP_S_KEY);
    PrintKey(NWK_S_KEY);
#endif /* KEY_EXTRACTABLE */
    MW_LOG(TS_OFF, VLEVEL_H, "###### IDs  ######\r\n");
    MW_LOG(TS_OFF, VLEVEL_H, "###### DevEui:      %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\r\n",
           HEX8(SeNvm->DevEui));
    MW_LOG(TS_OFF, VLEVEL_H, "###### AppEui:      %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\r\n",
           HEX8(SeNvm->JoinEui));
    return SECURE_ELEMENT_SUCCESS;
}
