...
int32_t RFW_TransmitLongPacket( uint16_t payload_size, uint32_t timeout, void (*TxLongPacketGetNextChunkCb) (uint8_t** buffer, uint8_t buffer_size) )
{
  ...
  RFW_MW_LOG( TS_ON, VLEVEL_H,  "RevID=%04X\r\n", LL_DBGMCU_GetRevisionID() );
        ...
        case MODEM_MSK:
        {
            if (RFWPacket.Init.Enable==1)
            {
              ...
              if (total_size> RADIO_BUF_SIZE)
              {
                ...
                RFW_MW_LOG( TS_ON, VLEVEL_H,  "Timeout=%d,\r\n", chunk_timeout);
...
int32_t RFW_TransmitInit(uint8_t* inOutBuffer, uint8_t size, uint8_t* outSize)
{
  ...
  {
    RFW_MW_LOG( TS_ON, VLEVEL_H, "RadioSend Oversize\r\n");
    status= -1;
  }
  else
  {
...
void RFW_ReceivePayload( void )
{
  ...
  if (RFW_GetPacketLength(&PayloadLength)==0)
  {
    ...
    if (packet_length<LONGPACKET_CHUNK_LENGTH_BYTES)
    {
      ...
      RFW_MW_LOG( TS_ON, VLEVEL_H,  "end packet in %dms\r\n", timeout);
...
static void RFW_TransmitLongPacket_TxChunkProcess( void )
{
  ...
  RFW_MW_LOG( TS_ON, VLEVEL_H,  "read_ptr=%d, write_ptr=%d, bytes_sent=%d, bytes_loaded=%d,remaining to send=%d\r\n",
  ...
  RFW_MW_LOG( TS_ON, VLEVEL_H,  "next chunk size=%d, new write ptr=%d\n\r",chunk_size+ crc_size,  (uint8_t)(chunk_size+ crc_size + write_ptr));
}
#endif
...
static int32_t RFW_GetPacketLength(uint16_t* PayloadLength)
{
    ...
    RFW_MW_LOG( TS_ON, VLEVEL_H,  "PayloadLength=%d,\r\n", *PayloadLength);
    return 0;
}
...
static void RFW_GetPayloadProcess( void )
{
    ...
    if (RFWPacket.LongPacketRemainingBytes>size)
    {
      ...
      /*intermediate chunk*/
      RFW_MW_LOG( TS_ON, VLEVEL_H,  "RxTxPldLen=0x%02X,\r\n",SUBGRF_ReadRegister(SUBGHZ_GRTXPLDLEN));
      RFW_MW_LOG( TS_ON, VLEVEL_H,  "RxAddrPtr=0x%02X,\r\n",read_ptr);
      RFW_MW_LOG( TS_ON, VLEVEL_H,  "offset= %d, size=%d, remaining=%d,\r\n",RFWPacket.RadioBufferOffset, size, RFWPacket.LongPacketRemainingBytes);
    ...
    else
    {
      ...
      /*last chunk*/
      RFW_MW_LOG( TS_ON, VLEVEL_H,  "LastChunk. offset= %d, size=%d, remaining=%d,\r\n",RFWPacket.RadioBufferOffset, size,RFWPacket.LongPacketRemainingBytes);
...
static void RFW_GetPayload( uint8_t Offset, uint8_t Length )
{
    ...
    /* CRC check*/
    RFW_MW_LOG( TS_ON, VLEVEL_H,  "crc_result= 0x%02X%02X, crc_payload=0x%02X%02X\r\n",crc_result[0],crc_result[1],ChunkBuffer[Length-2],ChunkBuffer[Length-1]);
